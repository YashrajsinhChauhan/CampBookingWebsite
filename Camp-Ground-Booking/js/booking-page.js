...
const generateReservationReceipt = (numberOfNights, name, email, cost) => {
    ...
    <div id = "receipt-container">
        ...
        <h1>Reservation #RES-${receiptNumber}</h1>
        <p>Name: <span class = "field-value">${name.toUpperCase()}</span></p>
        <p>Email: <span class = "field-value">${email}</span></p>
        <p>Numuber of Nights: <span class = "field-value">${numberOfNights}</span></p>
        <p>Nightly Rate: <span class = "field-value">$${cost[0]}</span></p>
        <p>Subtotal: <span class = "field-value">$${cost[1]}</span></p>
        <p>Tax: <span class = "field-value">$${cost[2]}</span></p>
        <p>Total: <span class = "field-value">$${cost[3]}</span></p>
...
const pageLoaded = () => {
    ...
    // 2. Fetch & Display the details of the corrosponding campsite
    getCampsiteList()

    // 3. highlight current page in the navigation bar
